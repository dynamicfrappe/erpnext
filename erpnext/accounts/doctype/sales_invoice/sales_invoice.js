...
erpnext.accounts.SalesInvoiceController = erpnext.selling.SellingController.extend({
    ...
    refresh: function(doc, dt, dn) {
        ...
        if (doc.docstatus == 1) {
            cur_frm.add_custom_button(__('Gross Profit'),
                () => {
                    debugger;
                    frappe.call({
                        "method": "erpnext.accounts.report.gross_profit.gross_profit.get_gross_profit",
                        "args": {
                        ...
                        callback: function(r) {
                            debugger;
                            var gross_profit = 0;
                            var revenue_margin = cur_frm.doc.revenue_margin;
                            if (r.message && r.message.length) {
                                gross_profit = r.message[16];
                            }
                            frappe.msgprint(__("Gross Profit :  {0} <br/> Revenue Margin :  {1}", [gross_profit, revenue_margin]));
                        }
                    });

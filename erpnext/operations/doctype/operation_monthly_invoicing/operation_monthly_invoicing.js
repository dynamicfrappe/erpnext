...
frappe.ui.form.on('Operation Monthly Invoicing', {
	...
	onload: function(frm) {
	 ...
	 create_invoice: function(frm) {
	 	debugger;
	 	let selected_rows = frm.get_field("monthly_details_data").grid.get_selected_children() ;
	...
	Calculate_totals:function(frm, cdt, cdn){
			...
			if (!d.gross_salary)
				d.gross_salary = 0 ;
			if (!d.social_insurance)
				d.social_insurance = 0 ;
			if (!d.laptop)
				d.laptop = 0 ;
			if (!d.ohs_courses)
				d.ohs_courses = 0 ;
			if (!d.medical_insurance)
		...
		if (d.employee)
		{
			frappe.call({
		        ...
		        callback:function(r){
		        	...
		        	debugger;
		        	if (r.message) {
		        		if (!r.message.outsource ||r.message.status != 'Active' || r.message.company !=  frm.doc.company)
		        			d.employee = ""
		        		frm.refresh_fields();
		        	}

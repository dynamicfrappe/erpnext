...
def get_attendance (device_name):
	...
	doc =  frappe.db.get_value('Device', device_name, ['name', 'ip' , 'port' , 'password' , 'is_active' , 'last_log_date' , 'last_connection' , 'last_error'], as_dict=1)
	if doc:		
		if doc.is_active:	
			# create ZK instance
			last_log_date = None
			...
			device = ZK(ip=str(doc.ip), port=int(doc.port), timeout=5,password=int(doc.password), force_udp=False, ommit_ping=False)
			...
			try:
				...
				if doc.last_log_date:
					records = [r for r in data_dict if r['timestamp'] > datetime.strptime(doc.last_log_date,'%Y-%m-%d %H:%M:%S')]
				else: records = data_dict
				...
				try:
					...
					if last_log_date:
							document.last_log_date = last_log_date
					if last_connection:
							document.last_connection = last_connection
					if last_error:

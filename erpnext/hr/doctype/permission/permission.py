...
class Permission(Document):
	...
	def validate(self):
		...
		#frappe.msgprint(asd)
		if not asd:
			frappe.throw("employee doesn't has department")
		...
		if not attendance_role.max_permission_minute_per_once:
			frappe.throw(_("Employee {} doesn't have any Permission in Attendance Rule \n Please Set Max Permission Minutes Per Once in Attendance Rule {}".format(self.employee, employee.attendance_role)))
		...
		if perviuos_permission[0].total_min + duration > attendance_roleattendance_role.max_permission_minutes  :
		...
		if duration > attendance_role.max_permission_minute_per_once:
			frappe.throw(_("Employee {} Cann't exceed the max Permission Minitues in Per Time \n Please Set Max Permission Minitues per time in Attendance Rule {}".format(self.employee,employee.attendance_role)))

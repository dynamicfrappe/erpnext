...
frappe.ui.form.on('Purchase Request', {
	...
	schedule_date:function(frm){
		...
		if (frm.doc.schedule_date <frm.doc.posting_date ){
	...
	get_items_from_sales_order: function(frm) {
		erpnext.utils.map_current_doc({
			method: "erpnext.selling.doctype.sales_order.sales_order.make_purchase_request",
				...
				callback: function(r) {
					if (!r.message) {
						...
						$.each(r.message, function(i, item) {
							var d = frappe.model.add_child(cur_frm.doc, "Purchase Request Item", "items");
		...
		frappe.prompt(
			{
				...
				get_query: () => {
					return{
						query: "erpnext.buying.doctype.purchase_request.purchase_request.get_default_supplier_query",
			...
			(values) => {
				frappe.model.open_mapped_doc({
					method: "erpnext.buying.doctype.purchase_request.purchase_request.make_purchase_order",
					frm: frm,
	...
	make_request_for_quotation: function(frm) {
		frappe.model.open_mapped_doc({
			method: "erpnext.buying.doctype.purchase_request.purchase_request.make_request_for_quotation",
			frm: frm,
			run_link_triggers: true
		});
	},
	...
	make_supplier_quotation: function(frm) {
		frappe.model.open_mapped_doc({
			method: "erpnext.buying.doctype.purchase_request.purchase_request.make_supplier_quotation",
			frm: frm
		});
	},
